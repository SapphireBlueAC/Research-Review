...
const bucket = getStorage().bucket("research-review-v2.firebasestorage.app");
...
const extractText = async (storagePath, fileType) => {
  ...
  if (fileType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
    ...
    throw new Error(`Unsupported file type: ${fileType}. Please upload .txt or .docx.`);
  }
};

const getAiAnalysis = async (textContent) => {
  
  // --- THIS IS YOUR PERSONAL API KEY (NOW READ SECURELY) ---
  const API_KEY = process.env.GEMINI_API_KEY; 
  // ---
  
  if (!API_KEY || API_KEY === "YOUR_GEMINI_API_KEY") {
    console.error("Gemini API key is missing. Did you create a .env file in the /functions folder?");
  ...
  3. "matches": An array of one or more "match" objects, where each match has "source_id" (a mock source, e.g., "wikipedia.org"), "percent" (a number), and a "snippets" array with one example snippet string.
...
exports.runPlagiarismCheck = onDocumentCreated("reports/{reportId}", async (event) => {
  ...
  try {
    ...
    finalReport = {
      ...aiResult,
      status: aiResult.status || "complete",
    };
  } catch (e) {
